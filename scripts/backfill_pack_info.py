...
def main():
    ...
    for row in candidates:
        ...
        unit_qty = result.get("unit_quantity")
        uom = result.get("unit_of_measure")
        converted = to_seatable_base(unit_qty, uom) if (unit_qty is not None and uom) else None
        with open(pending_path, "w", encoding="utf-8") as f:
            json.dump({
                "sp_row_id": sp_row_id,
                "sp_name": sp_name,
                "pack_size": result.get("pack_size"),
                "unit_quantity": converted[0] if converted else unit_qty,
                "unit_of_measure": converted[1] if converted else (uom.upper() if uom else None),
